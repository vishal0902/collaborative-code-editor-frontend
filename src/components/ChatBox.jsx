  ...
  return (
    <div className="flex flex-col bg-slate-900/95 backdrop-blur-sm justify-center min-h-fit p-4 rounded-xl border border-slate-700/50 shadow-2xl md:w-80 w-60">
      <div className="text-green-400 text-sm font-semibold mb-3 tracking-wide flex justify-between">
        <span className="align-middle">&#8226; Live Chat </span><span onClick={()=>setShowChatBox(false)} style={{userSelect: "none"}} className="border border-gray-500 bg-green-500 text-white hover:bg-red-900/70 px-2 pb-0.5 rounded-sm text-xs transition-all">&times;</span>
      </div>
        ...
        <div className="space-y-1">
          {!msgBox.length ?              
             <div className="text-slate-400 md:text-sm text-[0.8rem]  max-w-fit   transition-colors fixed bottom-15 md:bottom-18 justify-self-center">no messages</div>
              :
             msgBox?.map(({time, userName, userId, msg, color}, index) =>{
              
                if(localStorage.getItem('currentUserId') === userId) {
                  return (
                    <div key={time} className="flex flex-col space-y-0.5 justify-self-end">
                      { (index !== 0) ? (msgBox[index].userId !== msgBox[index-1].userId) && <div style={{color: color}} className="md:text-[0.7rem] text-[0.5rem] font-medium text-end mt-2">You</div> : <div style={{color: color}} className="md:text-[0.7rem] text-[0.5rem] font-medium text-end  mt-2">You</div>}
                      <div className="text-slate-700 bg-slate-100 md:text-[0.8rem] text-[0.6rem] rounded-lg px-3 py-1 max-w-fit text-wrap break-words border border-slate-600/30">{msg}</div>
                    </div>)
                } else {
                  return (
                    <div key={time} className="flex flex-col space-y-0.5">
                      { (index !== 0) ? 
                          ((msgBox[index].userId !== msgBox[index-1].userId)  && <div style={{color: color}}  className="text-slate-400 md:text-[0.7rem] text-[0.5rem] font-medium  mt-2">{userName}</div>) :
                          <div style={{color: color}}  className="text-slate-400 md:text-[0.7rem] text-[0.5rem] font-medium  mt-2">{userName}</div>
                         }
                      <div className="text-slate-200 bg-slate-700/60 md:text-[0.8rem] text-[0.6rem] rounded-lg px-3 py-1 max-w-fit text-wrap break-words border border-slate-600/30 hover:bg-slate-700/80 transition-colors">{msg}</div>
                    </div>)
                }
             }
