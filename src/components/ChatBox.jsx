  ...
  return (
    <div className="flex flex-col bg-slate-900/95 backdrop-blur-sm justify-center min-h-fit p-4 rounded-xl border border-slate-700/50 shadow-2xl md:w-80 w-60">
      <div className="text-green-400 text-sm font-semibold mb-3 tracking-wide flex justify-between">
        <span className="align-middle">&#8226; Live Chat </span><span onClick={()=>setShowChatBox(false)} style={{userSelect: "none"}} className="border border-gray-500 bg-green-500 text-white hover:bg-red-900/70 px-1 rounded-sm text-xs transition-all">&times;</span>
      </div>
        ...
        <div className="space-y-1">
          {!msgBox.length ?              
             ...
             msgBox?.map(([key, m], index) =>{
                ...
                if(localStorage.getItem('currentUserId') === m.userId) {
                  return (
                    <div key={key} className="flex flex-col space-y-0.5 justify-self-end">
                      { (index !== 0) ? (msgBox[index][1]?.userId !== msgBox[index-1][1]?.userId) && <div style={{color: m.color}} className="md:text-[0.7rem] text-[0.5rem] font-medium text-end mt-2">You</div> : <div style={{color: m.color}} className="md:text-[0.7rem] text-[0.5rem] font-medium text-end  mt-2">You</div>}
                  ...
                  return (
                    <div key={key} className="flex flex-col space-y-0.5">
                      { (index !== 0) ? 
                          ((msgBox[index][1]?.userId !== msgBox[index-1][1]?.userId)  && <div style={{color: m.color}}  className="text-slate-400 md:text-[0.7rem] text-[0.5rem] font-medium  mt-2">{m.userName}</div>) :
